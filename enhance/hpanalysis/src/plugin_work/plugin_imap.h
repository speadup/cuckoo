...
struct imap_data_deal{
	char *imap_data;			//存储待截取邮件数据
...
struct imap_record
{
	...
	struct plugin_feature *imap_feature;					//处理该动作时对应的特征码结构
	struct imap_data_deal deal_flag;
...
static int plugin_data_storage(const struct data_transit *dtts, struct imap_record *imap_dst, const int thread_num, const int plugin_id);
