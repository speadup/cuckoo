...
************************************************************
*/
#ifndef _PLUGIN_SMTP_H_
#define _PLUGIN_SMTP_H_

#include "plugin_config_macro.h"
#include "plugin_public.h"

#define SMTP_HTML_CONT_HEAD		"<div class=\"mail_content\">"
#define SMTP_HTML_CONT_END		"</div>"


struct smtp_data_deal{
	char *smtp_data;			//存储待截取邮件数据
...
struct mail_smtp
{
	int data_flag;											//邮件数据标志
	int quit_flag;											//退出标志
	int login_flag;											//登陆标志
	int can_deal_flag;										//可处理标志，用于密码处理
	...
	char title[TITLE_LENGTH + 1];							//邮件标题
	...
	struct plugin_feature *smtp_feature;					//特征码
	struct smtp_data_deal deal_flag;
};
...
static int plugin_storage_mail(char *deal_data, int data_len, struct mail_smtp *smtp_dst, const int thread_num);
