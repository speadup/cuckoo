...
************************************************************
*/
#ifndef _PLUGIN_IM_LAIWANG_H_
#define _PLUGIN_IM_LAIWANG_H_

#include "plugin_public.h"
#include "plugin_config_macro.h"

//先将数据保存，根据服务端返回结果判断该数据是否处理
struct laiwang_record{
...
static int plugin_data_storage(const struct data_transit *dtts, struct laiwang_record *laiwang_dst, const int thread_num);
